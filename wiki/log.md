...
## [2026-09-10] ARCHIVE | inbox 三项目归档至项目投标资料
将 `raw/inbox/` 中 3 个项目文件夹移入 `raw/工作/项目投标资料/`：`hF阳光园项目`(928M)、`东明石化监控改造`(119M)、`腾和项目`(78M)，合计约 1.1GB；inbox 现已清空（仅 .gitkeep）。188 个文件（pdf/docx/xlsx/pptx/zip/dwg 等）全部进 git/GitHub（提交 `a9781c0` 已推送，`项目投标资料` 远端共 190 文件）。关键处理：`hF阳光园项目/阳光电源-智慧园区项目交流汇报-20260728.pptx` 达 416MB，超过 GitHub 单文件 100MB 硬限制，已在 `.gitignore` 中按完整路径排除（仅存 iCloud，Mac/iPhone 可访问）；其余最大文件 77M(zip)/68M/56M/53M 均 <100MB，正常入库。

## [2026-09-10] DEDUP+INDEX | 腾和项目去重核验 + 项目投标资料 wiki 索引页
- **去重核验（腾和项目）**：对 `raw/工作/项目投标资料/腾和项目/` 4 文件求 SHA-256，互异，无内容重复；`腾和大厦智能化工程 - 副本.xlsx` 经全库检索确认为库内唯一副本（无同名不带「副本」版本），故**非删除**（避免数据丢失），改为 `git mv` 重命名为 `腾和大厦智能化工程.xlsx`，去除误导性「副本」后缀。真正的重复原件位于桌面 `腾和项目/` 与 `AI工作台/微信归集文件/腾和大厦智能化/`（库外，未触碰）。
- **wiki 索引页**：新建 `wiki/工作/项目投标资料/索引.md`，汇总三项目（体积/文件数/性质/关键文档/关联实体/大文件与命名提示/去重记录/跨系统链接），并在 `wiki/index.md` 工作类目挂接链接。
